"""
Modelo para gestión de clientes y proveedores - VERSIÓN CORREGIDA
"""
...
class ClienteProveedorModel:
    ...
    def obtener_clientes(busqueda="", estado=None, deuda=None, limit=10, offset=0):
        ...
        try:
            ...
            sql = """
            SELECT c.*,
                   COALESCE(SUM(crd.saldo_pendiente), 0) as deuda_total,
                   COUNT(DISTINCT v.id) as total_ventas,
                   COALESCE(SUM(v.total), 0) as monto_total_ventas,
                   CASE 
                       WHEN COALESCE(SUM(crd.saldo_pendiente), 0) = 0 THEN 'activo'
                       WHEN COALESCE(SUM(crd.saldo_pendiente), 0) > 50000 THEN 'moroso'
                       ELSE 'activo'
                   END as estado_actual
            FROM cliente c
            LEFT JOIN ventas v ON c.cedula = v.cliente_cedula
            LEFT JOIN creditos crd ON c.cedula = crd.cliente_cedula AND crd.estado = 'pendiente'
            WHERE 1=1
            """
            ...
            # Agrupar
            sql += " GROUP BY c.cedula, c.nombre, c.telefono, c.correo, c.direccion"
            
            # Filtro por deuda (después del GROUP BY)
            if deuda == "sin":
                sql += " HAVING deuda_total = 0"
            elif deuda == "pequena":
                sql += " HAVING deuda_total > 0 AND deuda_total < 50000"
            elif deuda == "grande":
                sql += " HAVING deuda_total >= 50000"
            
            # Filtro por estado (después del GROUP BY)
            if estado == "activo":
                sql += " HAVING estado_actual = 'activo'"
            elif estado == "moroso":
                sql += " HAVING estado_actual = 'moroso'"
    ...
    def obtener_cliente_por_cedula(cedula):
        ...
        try:
            ...
            LEFT JOIN creditos crd ON c.cedula = crd.cliente_cedula AND crd.estado = 'pendiente'
            WHERE c.cedula = %s
            GROUP BY c.cedula, c.nombre, c.telefono, c.correo, c.direccion
            """
    ...
    @staticmethod
    def actualizar_cliente(cedula_original, cedula, nombre, telefono, correo, direccion):
    ...
    def obtener_historial_cliente(cedula):
        """Obtener historial completo de un cliente - VERSIÓN CORREGIDA"""
        conn = None
        cursor = None
        try:
            conn = db.get_connection()
            ...
            # Obtener ventas del cliente - Convertir fechas y horas a strings
            sql_ventas = """
            SELECT 
                v.id,
                v.numero_venta,
            ...
            GROUP BY v.id, v.numero_venta, v.fecha_dia, v.fecha_hora, v.nombre_cliente, 
                     v.tipo_pago, v.cliente_cedula, v.subtotal, v.descuento, v.total
    ...
    def actualizar_credito(credito_id, datos):
        ...
        try:
            ...
            cursor.execute(sql, (
                ...
                datos.get('saldo_pendiente', 0),
                datos.get('dias_credito', 30),
                datos.get('fecha_vencimiento'),
                datos.get('estado', 'pendiente'),
                datos.get('abonos_realizados', 0),
                datos.get('ultimo_pago'),
            ...
            logger.info(f"Crédito {credito_id} actualizado")
            return True, "Crédito actualizado exitosamente"
            
        except Exception as e:
            if conn:
                conn.rollback()
            logger.error(f"Error al actualizar crédito {credito_id}: {str(e)}")
            return False, f"Error al actualizar crédito: {str(e)}"
        finally:
            if cursor:
                cursor.close()
            if conn:
    ...
    @staticmethod
    def registrar_abono_credito(credito_id, monto_abono, fecha_abono, observaciones=""):
        ...
        try:
            ...
            cursor.execute("SELECT saldo_pendiente, abonos_realizados FROM creditos WHERE id = %s", (credito_id,))
            ...
            logger.info(f"Abono registrado para crédito {credito_id}: {monto_abono}")
    ...
    def obtener_proveedores(busqueda="", estado=None, limit=10, offset=0):
        ...
        try:
            ...
            # Consulta con el campo producto
            sql = """
            SELECT 
                p.telefono,
                p.nombre_empresa,
    ...
    def crear_proveedor_con_productos(telefono, nombre_empresa, nombre_proveedor, correo=None, estado='activo', productos=None):
        ...
        try:
            ...
            # Crear proveedor
            sql_proveedor = """
            INSERT INTO proveedor (telefono, nombre_empresa, nombre_proveedor, correo, estado, producto)
            VALUES (%s, %s, %s, %s, %s, %s)
            """
    ...
    def obtener_historial_proveedor(telefono):
        """Obtener historial completo de un proveedor - VERSIÓN SIMPLIFICADA"""
        conn = None
        cursor = None
        try:
            conn = db.get_connection()
            ...
            historial = {
                ...
                'resumen': {
                    'total_productos': proveedor_info.get('total_productos', 0),
